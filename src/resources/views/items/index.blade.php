...
<div class="main-box">
    <div class="main-box-top-wrapper">
        <div class="main-box-top">
            <a class="main-box-top__item {{ request('tab') === 'recommend' ? 'active' : '' }}" 
            href="{{ route('items.index', ['tab' => 'recommend']) }}">おすすめ</a>
            <a class="main-box-top__item {{ request('tab') === 'favorites' ? 'active' : '' }}" 
            href="{{ route('items.index', ['tab' => 'favorites']) }}">マイリスト</a>
        </div>
    </div>
    
    <div class="main-box-bottom-wrapper">
        @if($tab === 'favorites' && $items->isEmpty())
        <p>お気に入りのアイテムがありません。</p>
        @else
